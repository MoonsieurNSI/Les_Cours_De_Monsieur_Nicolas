...
> [!IMPORTANT]
> Le tutoriel suivant a été écrit sur `LinuxMint Debian Edition` mais devrait fonctionner (avec plus ou moins d'ajustements) sur les distributions Linux basées sur [Debian] (./img/debianTree.txt)(en fait celles utilisant nativement `apt` comme gestionnaire de packages):
>  #### Debian Based:
